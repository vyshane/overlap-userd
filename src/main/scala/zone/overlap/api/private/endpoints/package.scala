...
import zone.overlap.api.`private`.user.{FindUserByIdRequest, FindUserByIdResponse, User}
...
package object Endpoints {
  ...
  def findUserById(queryDb: String => Option[UserRecord])
  ...
  // TODO: Convert all of the user fields
  private def userRecordToProto(record: UserRecord): User =
    User(record.id, record.firstName, record.lastName, record.email)
}
